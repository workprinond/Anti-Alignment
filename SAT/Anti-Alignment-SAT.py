...
from SAT.distancemearuement import edit_distance,levenshtein
from SAT.Petrinet_to_sat import petri_net_to_SAT
from SAT.log_to_SAT import log_to_SAT
from SAT.variablegenerator import VariablesGenerator as vg
...
class AntiAlignmentSAT:
    '''
    The AntiAlignmentSAT class is a  class that formes the SAT structure:
    -   Anti-Alignment
    ...
    def __compute_distance(self, artefact, wait_transition):
        ...
        if self.__distance_type == EDIT_DISTANCE:
            return edit_distance(artefact, self.__transitions, self.__silent_transitions, self.__vars, len(self.__traces),
                                                  self.__size_of_run,
...
def antiAlignmentPrecision(net, m0,mf, log, epsilon,distance="edit",nbMaxOfTrials=10):
    ...
    computeAntiAlignment=AntiAlignmentSAT(reachFinal=True)
